describe("hx-swap-oob attribute", function () {
  ...
  it("handles more than one oob swap properly", function () {
    this.server.respondWith(
      "GET",
      "/test",
      "Clicked<div id='d1' hx-swap-oob='true'>Swapped1</div><div id='d2' hx-swap-oob='true'>Swapped2</div>"
    );
    var div = make('<div hx-get="/test">click me</div>');
    make('<div id="d1"></div>');
    make('<div id="d2"></div>');
  ...
  it("oob swaps can be nested in content", function () {
    this.server.respondWith(
      "GET",
      "/test",
      "<div>Clicked<div id='d1' foo='bar' hx-swap-oob='innerHTML'>Swapped6</div></div>"
    );
    var div = make('<div hx-get="/test">click me</div>');
    make('<div id="d1"></div>');
    div.click();
  ...
  it("swaps into all targets that match the selector (innerHTML)", function () {
    this.server.respondWith(
      "GET",
      "/test",
      "<div>Clicked</div><div class='target' hx-swap-oob='innerHTML:.target'>Swapped8</div>"
    );
    var div = make('<div hx-get="/test">click me</div>');
