describe("hx-target attribute", function () {
  ...
  it("targets a `closest` element properly", function () {
    this.server.respondWith("GET", "/test", "Clicked!");
    var div1 = make(
      '<div><p><i><button id="b1" hx-target="closest div" hx-get="/test">Click Me!</button></i></p></div>'
    );
    var btn = byId("b1");
    btn.click();
    this.server.respond();
  ...
  it("targets a `find` element properly", function () {
    this.server.respondWith("GET", "/test", "Clicked!");
    var div1 = make(
      '<div hx-target="find span" hx-get="/test">Click Me! <div><span id="s1"></span><span id="s2"></span></div></div>'
    );
    div1.click();
    this.server.respond();
    var span1 = byId("s1");
