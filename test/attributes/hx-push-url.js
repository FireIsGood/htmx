describe("hx-push-url attribute", function () {
  ...
  it("restore should return old value", function () {
    this.server.respondWith(
      "GET",
      "/test1",
      '<div id="d2" hx-push-url="true" hx-get="/test2" hx-swap="outerHTML settle:0">test1</div>'
    );
    this.server.respondWith(
      "GET",
      "/test2",
      '<div id="d3" hx-push-url="true" hx-get="/test3" hx-swap="outerHTML settle:0">test2</div>'
    );
  ...
  it("cache should only store 10 entries", function () {
    ...
    this.server.respondWith("GET", /test.*/, function (xhr) {
      ...
      xhr.respond(
        200,
        {},
        '<div id="d1" hx-push-url="true" hx-get="/test' + x + '" hx-swap="outerHTML settle:0"></div>'
      );
    });
    getWorkArea().innerHTML.should.be.equal("");
  ...
  it("should include parameters on a get", function () {
    ...
    try {
      ...
      var form = make(
        '<form hx-trigger="click" hx-push-url="true" hx-get="/test"><input type="hidden" name="foo" value="bar"/>first</form>'
      );
      form.click();
      this.server.respond();
