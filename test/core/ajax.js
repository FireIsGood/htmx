describe("Core htmx AJAX Tests", function () {
  ...
  it("properly handles multiple select input", function () {
    ...
    var form = make(
      '<form hx-post="/test" hx-trigger="click">' +
  ...
  it("properly handles multiple email input", function () {
    ...
    var form = make(
      '<form hx-post="/test" hx-trigger="click">' + '<input id="multiEmail" name="multiEmail" multiple>' + "</form>"
    );
  ...
  it("script nodes can define global functions", function () {
    try {
      ...
      this.server.respondWith(
        "GET",
        "/test",
        "<script type='text/javascript'>foo.bar = function() { return 42 }</script>"
      );
      var div = make("<div hx-get='/test'></div>");
      div.click();
      this.server.respond();
  ...
  it("script nodes evaluate after swap", function () {
    ...
    try {
      this.server.respondWith(
        "GET",
        "/test",
        "<div><script>callGlobal()</script><div id='d1'>After settle...</div> </div>"
      );
      var div = make("<div hx-get='/test'></div>");
      div.click();
      this.server.respond();
  ...
  it("multipart/form-data encoding works", function () {
    ...
    var form = make(
      "<form hx-post='/test' hx-encoding='multipart/form-data' hx-trigger='click'>" +
  ...
  it("first title tag outside svg title tags updates title", function () {
    ...
    this.server.respondWith("GET", "/test", function (xhr) {
      xhr.respond(
        ...
        "<title class=''>" +
          newTitle +
          "</title><svg class=''><title>foo</title></svg>Clicked!<title class=''>x</title>"
      );
    });
    var btn = make('<button hx-get="/test">Click Me!</button>');
    btn.click();
  ...
  it("properly handles clicked submit button with a value outside a htmx form", function () {
    ...
    make(
      '<form id="externalForm" hx-post="/test">' +
        '<input type="text" name="t1" value="textValue">' +
        "</form>" +
  ...
  it("handles form post with button formmethod dialog properly", function () {
    ...
    make(
      '<dialog><form hx-post="/test"><button id="submit" formmethod="dialog" name="foo" value="bar">submit</button></form></dialog>'
    );
