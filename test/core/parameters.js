describe("Core htmx Parameter Handling", function () {
  ...
  it("Double values are included as array", function () {
    var form = make(
      '<form><input id="i1" name="foo" value="bar"/><input id="i2" name="do" value="rey"/><input id="i2" name="do" value="rey"/></form>'
    );
  ...
  it("Double values are included as array in correct order", function () {
    var form = make(
      '<form><input id="i1" name="foo" value="bar"/><input id="i2" name="do" value="rey1"/><input id="i3" name="do" value="rey2"/></form>'
    );
  ...
  it("Double empty values are included as array in correct order", function () {
    var form = make(
      '<form><input id="i1" name="do" value=""/><input id="i2" name="do" value="rey"/><input id="i3" name="do" value=""/></form>'
    );
    var vals = htmx._("getInputValues")(byId("i3")).values;
  ...
  it("hx-include works with form", function () {
    var form = make(
      '<form id="f1"><input id="i1" name="foo" value="bar"/><input id="i2" name="do" value="rey"/><input id="i2" name="do" value="rey"/></form>'
    );
  ...
  it("correctly URL escapes values", function () {
    ...
    htmx
      ._("urlEncode")({ foo: "bar", do: ["rey", "blah"] })
      .should.equal("foo=bar&do=rey&do=blah");
  });

  it("form includes last focused button", function (done) {
    var form = make(
      '<form hx-get="/foo"><input id="i1" name="foo" value="bar"/><input id="i2" name="do" value="rey"/><button id="b1" name="btn" value="bar"></button></form>'
    );
    var input = byId("i1");
    var button = byId("b1");
  ...
  it("form includes last focused submit", function (done) {
    var form = make(
      '<form hx-get="/foo"><input id="i1" name="foo" value="bar"/><input id="i2" name="do" value="rey"/><input type="submit" id="s1" name="s1" value="bar"/></form>'
    );
    var input = byId("i1");
    var button = byId("s1");
    // Listen for focusin on form as it'll bubble up from the button, and htmx binds on the form itself
  ...
  it("form includes button name and value if button has nested elements when clicked", function () {
    var form = make(
      '<form hx-get="/foo"><input id="i1" name="foo" value="bar"/><button type="submit" id="btn1" name="do" value="rey"><div id="div1"><span id="span1"></span></div></button></form>'
    );
  ...
  it("it puts GET params in the URL by default", function () {
    ...
    var form = make(
      '<form hx-trigger="click" hx-get="/test"><input name="i1" value="value"/><button id="b1">Click Me!</button></form>'
    );
    form.click();
    this.server.respond();
  ...
  it("it puts DELETE params in the body by default", function () {
    ...
    var form = make(
      '<form hx-trigger="click" hx-delete="/test"><input name="i1" value="value"/><button id="b1">Click Me!</button></form>'
    );
    form.click();
    this.server.respond();
    form.innerHTML.should.equal("Clicked!");
  ...
  it("Input within disabled fieldset is excluded", function () {
    var input = make(
      '<form><input name="foo" value="bar"/><fieldset disabled><input name="do" value="rey"/></fieldset></form>'
    );
    var vals = htmx._("getInputValues")(input, "get").values;
    vals["foo"].should.equal("bar");
    should.equal(vals["do"], undefined);
