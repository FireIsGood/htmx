describe("sse extension", function () {
  ...
  it("handles basic sse triggering", function () {
    ...
    var div = make(
      '<div hx-ext="sse" sse-connect="/foo">' +
  ...
  it("does not trigger events that arent named", function () {
    ...
    var div = make(
      '<div hx-ext="sse" sse-connect="/foo">' + '<div id="d1" hx-trigger="sse:e1" hx-get="/d1">div1</div>' + "</div>"
    );
  ...
  it("does not trigger events not on descendents", function () {
    ...
    var div = make(
      '<div hx-ext="sse" sse-connect="/foo"></div>' + '<div id="d1" hx-trigger="sse:e1" hx-get="/d1">div1</div>'
    );
  ...
  it("is closed after removal, hx-trigger", function () {
    ...
    var div = make(
      '<div hx-get="/test" hx-swap="outerHTML" hx-ext="sse" sse-connect="/foo">' +
        '<div id="d1" hx-trigger="sse:e1" hx-get="/d1">div1</div>' +
        "</div>"
    );
    div.click();
  ...
  it("is not listening for events after hx-swap element removed", function () {
    var div = make(
      '<div hx-ext="sse" sse-connect="/foo">' + '<div id="d1" hx-swap="outerHTML" sse-swap="e1">div1</div>' + "</div>"
    );
  ...
  it("initializes connections in swapped content", function () {
    this.server.respondWith(
      "GET",
      "/d1",
      '<div><div sse-connect="/foo"><div id="d2" hx-trigger="sse:e2" hx-get="/d2">div2</div></div></div>'
    );
    this.server.respondWith("GET", "/d2", "div2 updated");
