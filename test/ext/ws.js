describe("web-sockets extension", function () {
  ...
  function mockWebsocket() {
    ...
    var mockSocketServer = {
      ...
      clients: function () {
        // Replicate old mock-socket syntax to avoid huge file diff to merge
  ...
  it("sends data to the server with specific trigger", function () {
    var div = make(
      '<div hx-ext="ws" ws-connect="ws://localhost:8080"><div hx-trigger="click" ws-send id="d1">div1</div></div>'
    );
    this.tickMock();
  ...
  it("sends data to the server with polling trigger", function () {
    var div = make(
      '<div hx-ext="ws" ws-connect="ws://localhost:8080"><div hx-trigger="every 1s" ws-send id="d1">div1</div></div>'
    );
    this.tickMock();
  ...
  it("sends expected headers to the server", function () {
    var div = make(
      '<div hx-ext="ws" ws-connect="ws://localhost:8080"><button hx-trigger="click" hx-target="#target" ws-send id="d1" name="d1-name">div1</button><output id="target"></output></div>'
    );
    this.tickMock();
  ...
  it("handles message from the server", function () {
    var div = make(
      '<div hx-ext="ws" ws-connect="ws://localhost:8080"><div id="d1">div1</div><div id="d2">div2</div></div>'
    );
    this.tickMock();
  ...
  it("raises lifecycle events (connecting, open, close) in correct order", function () {
    ...
    var handler = function (evt) {
      handledEventTypes.push(evt.detail.event.type);
    };
  ...
  it("sends data to the server with non-htmx form + submit button & value", function () {
    make(
      '<form hx-ext="ws" ws-connect="ws://localhost:8080" ws-send>' +
