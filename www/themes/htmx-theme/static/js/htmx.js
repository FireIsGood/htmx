...
(function (root, factory) {
  ...
  return (function () {
      ...
      if (tokens[0] === "[") {
        ...
        while (tokens.length > 0) {
          ...
          if (isPossibleRelativeReference(token, last, paramName)) {
            conditionalSource +=
              "((" + paramName + "." + token + ") ? (" + paramName + "." + token + ") : (window." + token + "))";
          } else {
    ...
    function boostElement(elt, nodeData, triggerSpecs) {
      if (
        (elt.tagName === "A" && isLocalLink(elt) && (elt.target === "" || elt.target === "_self")) ||
        elt.tagName === "FORM"
      ) {
    ...
    function ignoreBoostedAnchorCtrlClick(elt, evt) {
      return (
        getInternalData(elt).boosted && elt.tagName === "A" && evt.type === "click" && (evt.ctrlKey || evt.metaKey)
      );
    }
    ...
    function getInputValues(elt, verb) {
      ...
      var validate =
        (matches(elt, "form") && elt.noValidate !== true) || getAttributeValue(elt, "hx-validate") === "true";
    ...
    function handleAjaxResponse(elt, responseInfo) {
      ...
      var beforeSwapDetails = mergeObjects(
        { shouldSwap: shouldSwap, serverResponse: serverResponse, isError: isError, ignoreTitle: ignoreTitle },
        responseInfo
      );
        ...
        var doSwap = function () {
          try {
            ...
            if (selectionInfo.elt && !bodyContains(selectionInfo.elt) && getRawAttribute(selectionInfo.elt, "id")) {
              ...
              var focusOptions = {
                preventScroll:
                  swapSpec.focusScroll !== undefined ? !swapSpec.focusScroll : !htmx.config.defaultFocusScroll,
              };
              if (newActiveElt) {
                // @ts-ignore
