...
function headers(request) {
  return request
    ...
    .split("\r\n")
    .filter((h) => h.toLowerCase().startsWith("hx-"))
    .map((h) => h.split(": "))
...
htmx.on("htmx:beforeSwap", function (event) {
  if (document.getElementById("request-count")) {
    requestId++;
    pushActivityChip(
      `${server.getHTTPMethod(event.detail.xhr)} ${event.detail.xhr.url}`,
      `req-${requestId}`,
      demoResponseTemplate(event.detail)
    );
...
function pushActivityChip(name, id, content) {
  document
    .getElementById("demo-timeline")
    .insertAdjacentHTML(
      "afterbegin",
      `<li id="${id}-link"><a onclick="showTimelineEntry('${id}')" style="cursor: pointer">${name}</a></li>`
    );
