describe("hx-trigger attribute", function () {
  ...
  it("parses spec strings", function () {
    var specExamples = {
      ...
      "event1, event2": [{ trigger: "event1" }, { trigger: "event2" }],
      "event1 once, event2 changed": [
        { trigger: "event1", once: true },
        { trigger: "event2", changed: true },
      ],
    ...
    for (var specString in specExamples) {
      ...
      spec.should.deep.equal(
        specExamples[specString],
        "Found : " +
          JSON.stringify(spec) +
          ", expected : " +
          JSON.stringify(specExamples[specString]) +
          " for spec: " +
          specString
      );
    }
  ...
  it("from clause works with find clause", function () {
    ...
    var div1 = make(
      '<div hx-trigger="click from:find a" hx-target="#a1" hx-get="/test"><a id="a1">Requests: 0</a></div>'
    );
    var a1 = byId("a1");
    a1.innerHTML.should.equal("Requests: 0");
    a1.click();
  ...
  it("event listeners on other elements are removed when an element is swapped out", function () {
    ...
    var div1 = make(
      '<div hx-get="/test2">' + '<div id="d2" hx-trigger="click from:body" hx-get="/test">Requests: 0</div>' + "</div>"
    );
    var div2 = byId("d2");
  ...
  it("consume prevents event propagation", function () {
    ...
    var div = make(
      "<div hx-trigger='click' hx-get='/foo'>" +
  ...
  it("A throttle of 0 does not multiple requests from happening", function (done) {
    ...
    server.respondWith("GET", "/bar", "bar");
    var div = make("<div hx-trigger='click throttle:0ms' hx-get='/test'></div>");
  ...
  it("A 0 delay does not delay the request", function (done) {
    ...
    this.server.respondWith("GET", "/bar", "bar");
    var div = make("<div hx-trigger='click delay:0ms' hx-get='/test'></div>");
  ...
  it("reveal event works when triggered by window", function () {
    this.server.respondWith("GET", "/test1", "test 1");
    var div = make(
      '<div hx-get="/test1" hx-trigger="revealed" style="position: fixed; top: 1px; left: 1px; border: 3px solid red">foo</div>'
    );
  ...
  it("correctly reuses trigger specs from the cache if defined", function () {
    ...
    spec.should.deep.equal(
      expectedSpec,
      "Found : " + JSON.stringify(spec) + ", expected : " + JSON.stringify(expectedSpec) + " for spec: " + triggerStr
    );
    ...
    spec2.should.deep.equal(
      spec,
      "Found : " + JSON.stringify(spec) + ", expected : " + JSON.stringify(spec2) + " for cached spec: " + triggerStr
    );
