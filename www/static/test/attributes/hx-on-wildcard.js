describe("hx-on:* attribute", function () {
  ...
  it("can modify a parameter via htmx:configRequest", function () {
    ...
    var btn = make(
      "<button hx-on:htmx:config-request='event.detail.parameters.foo = \"bar\"' hx-post='/test'>Foo</button>"
    );
    btn.click();
    this.server.respond();
  ...
  it("expands :: shorthand into htmx:", function () {
    ...
    var btn = make(
      "<button hx-on::config-request='event.detail.parameters.foo = \"bar\"' hx-post='/test'>Foo</button>"
    );
    btn.click();
    this.server.respond();
    btn.innerText.should.equal("bar");
  ...
  it("can cancel an event via preventDefault for htmx:config-request", function () {
    ...
    var btn = make(
      "<button hx-on:htmx:config-request='event.preventDefault()' hx-post='/test' hx-swap='outerHTML'>Foo</button>"
    );
    btn.click();
    this.server.respond();
  ...
  it("can respond to data-hx-on", function () {
    ...
    var btn = make(
      "<button data-hx-on:htmx:config-request='event.detail.parameters.foo = \"bar\"' hx-post='/test'>Foo</button>"
    );
    btn.click();
    this.server.respond();
    btn.innerText.should.equal("bar");
  ...
  it("can handle multi-line JSON", function () {
    ...
    var btn = make(
      "<button hx-on:htmx:config-request='window.elt = {foo: true,\n" +
