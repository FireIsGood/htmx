describe("hx-swap attribute", function () {
  ...
  it("works immediately with no swap delay", function (done) {
    this.server.respondWith("GET", "/test", "Clicked!");
    var div = make("<div hx-get='/test' hx-swap='innerHTML swap:0ms'></div>");
    div.click();
    this.server.respond();
    div.innerText.should.equal("Clicked!");
    done();
  ...
  it("works with a settle delay", function (done) {
    this.server.respondWith(
      "GET",
      "/test",
      "<div id='d1' class='foo' hx-get='/test' hx-swap='outerHTML settle:10ms'></div>"
    );
  ...
  it("works with no settle delay", function (done) {
    ...
    var div = make("<div id='d1' hx-get='/test' hx-swap='outerHTML settle:0ms'></div>");
    div.click();
    this.server.respond();
    div.classList.contains("foo").should.equal(false);
    setTimeout(function () {
