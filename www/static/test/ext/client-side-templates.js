describe("client-side-templates extension", function () {
  ...
  it("works on mustache array template", function () {
    this.server.respondWith("GET", "/test", '{"foo":"bar"}');
    var btn = make(
      '<button hx-get="/test" hx-ext="client-side-templates" mustache-array-template="mt1">Click Me!</button>'
    );
  ...
  it("works on basic handlebars template", function () {
    this.server.respondWith("GET", "/test", '{"foo":"bar"}');
    var btn = make(
      '<button hx-get="/test" hx-ext="client-side-templates" handlebars-template="hb1">Click Me!</button>'
    );
  ...
  it("works on handlebars array template", function () {
    this.server.respondWith("GET", "/test", '[{"foo":"bar"}]');
    var btn = make(
      '<button hx-get="/test" hx-ext="client-side-templates" handlebars-array-template="hb1">Click Me!</button>'
    );
    make('<script id="hb1" type="text/x-handlebars-template">*{{#.}}{{foo}}{{/.}}*</script>');
    btn.click();
    this.server.respond();
  ...
  it("works on basic xslt template", function () {
    ...
    make(
      '<script id="mt1" type="application/xml">' +
