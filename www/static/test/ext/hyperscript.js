describe("hyperscript integration", function () {
  ...
  it("can handle htmx error events", function () {
    ...
    var btn = make(
      '<button _="on htmx:error(errorInfo) put errorInfo.error into #d1.innerHTML" hx-get="/test">Click Me!</button>'
    );
    htmx.trigger(btn, "htmx:load");
    btn.click();
    this.server.respond();
  ...
  it("hyperscript in non-htmx annotated nodes is evaluated", function () {
    this.server.respondWith(
      "GET",
      "/test",
      "<div><div><div id='d1' _='on click put \"Clicked...\" into my.innerHTML'></div></div></div>"
    );
