describe("loading states extension", function () {
  ...
  it("works with multiple directives", function () {
    ...
    var btn = make('<button hx-get="/test" hx-ext="loading-states">Click Me!</button>');
    var element = make(
      '<button data-loading-aria-busy data-loading-class="loading" data-loading-class-remove="not-loading" class="not-loading">'
    );
    btn.click();
    element.should.have.attribute("aria-busy", "true");
  ...
  it("works with custom targets", function () {
    this.server.respondWith("GET", "/test", "Clicked!");
    var btn = make(
      '<button hx-get="/test" hx-ext="loading-states" data-loading-target="#loader" data-loading-class="test">Click Me!</button>'
    );
  ...
  it("history restore should not have loading states in content", function () {
    ...
    this.server.respondWith(
      "GET",
      "/test1",
      '<button id="d2" hx-push-url="true" hx-get="/test2" hx-swap="outerHTML settle:0" data-loading-disable>test1</button>'
    );
    this.server.respondWith(
      "GET",
      "/test2",
      '<button id="d3" hx-push-url="true" hx-get="/test3" hx-swap="outerHTML settle:0" data-loading-disable>test2</button>'
    );

    make(
      '<div hx-ext="loading-states"><button id="d1" hx-push-url="true" hx-get="/test1" hx-swap="outerHTML settle:0" data-loading-disable>init</button></div>'
    );
