describe("Core htmx Events", function () {
  ...
  it("htmx:configRequest on form gives access to submit event", function () {
    ...
    try {
      ...
      make(
        '<div hx-target="this" hx-boost="true"><form action="/test" method="post"><button type="submit" id="b1">Submit</button><button type="submit" id="b2">Submit</button></form></div>'
      );
      var btn = byId("b1");
      btn.click();
      this.server.respond();
  ...
  it("htmx:afterSettle is called multiple times when doing OOB outerHTML swaps", function () {
    ...
    try {
      ...
      var div = make(
        "<button id='button' hx-post='/test' hx-target='#t'>Foo</button><div id='t'></div><div id='t1'></div><div id='t2'></div>"
      );
